...
func(user *UserGroup) GetHTTPHandler() []*handler.HTTPHandler {
	log.Printf("Http Handler Redirecting to object based on path ..guser_groups.")
	return []*handler.HTTPHandler{
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "usergroup/{id}", Func: user.GetByID},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPost, Path: "usergroup", Func: user.Create},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPut, Path: "usergroup/{id}", Func: user.Update},
...
func (user *UserGroup) Create(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
...
func (user *UserGroup) Update(w http.ResponseWriter, r *http.Request) {
	//log.Printf("Handler")
...
func (user *UserGroup) Delete(w http.ResponseWriter, r *http.Request) {
	var payload string
	id, err := strconv.ParseInt(chi.URLParam(r, "id"), 10, 64)
	for {
		if nil != err {
...
func (user *UserGroup) GetAll(w http.ResponseWriter, r *http.Request) {
